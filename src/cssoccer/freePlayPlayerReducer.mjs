...
export function projectCssoccerFreePlayZonalPlayerVisit(input = {}) {
  ...
  const targetPoint = input.targetOverride === null
    ? null
    : requireF32Point(input.targetOverride, "zonal player visit target override");
  const targetOverride = targetPoint === null
    ? null
...
export function stepCssoccerFreePlayOpeningTeamContinuation(input = {}) {
  ...
  return input.players.map((player) => {
    ...
    if (continuingSupportRun) {
      // A live I_RUN_ON visit executes the already-installed RUN_ACT route.
      // Re-running get_there_time against the shrinking offset can retain an
      // extra go_cnt even while position remains exact, which delays the
      // terminal run_action/find_zonal_target fallthrough by one tick.
    ...
    const continuingIntercept = player.intelligence.move === 1
      && player.intelligence.count > 1
      && player.action.action.value === CSSOCCER_NATIVE_ACTIONS.RUN
      && player.liveMotion.goCount > 1;
    if (continuingIntercept) {
      // A centre-pass collection changes possession before this suffix visit,
      // but it does not cancel an already-busy I_INTERCEPT. Native runs the
      // installed action and decrements its counters; replanning this player
      // as an ordinary zonal opponent overwrites the correct first-team pass.
      return continueSourceInterceptJourney(current, {
        ballPosition,
        nextTick: input.nextTick,
      });
    }
...
function continueSourceInterceptJourney(current, {
  ballPosition,
  nextTick,
}) {
...
export function stepCssoccerFreePlayTeamJourneyContinuation(input = {}) {
  ...
  if (
    !Array.isArray(input.sourceDecisionPlayers)
    || input.sourceDecisionPlayers.length !== input.players.length
    || input.sourceDecisionPlayers.some((player, index) => (
      player.id !== input.players[index].id
    ))
  ) {
    throw new Error("Team journey continuation lost its source-ordered decision frame.");
  }
  if (!Number.isSafeInteger(input.logicCount) || input.logicCount < 0) {
  ...
  if (typeof input.restartResetIdeas !== "boolean") {
    throw new TypeError("Team journey restartResetIdeas must be boolean.");
  }
  const tactics = currentTacticsState(input.tactics);
  const rates = currentRateMap(input.teamRates, input.players);
  ...
  if (
    input.zoneBallPosition === null
    && ["collect", "hold", "rebound"].includes(zoneAnchor.interaction)
  ) {
  ...
  const zones = input.zoneState === null
    ...
    : assertCssoccerZoneState(input.zoneState);
  ...
  const dribblePlayers = input.sourceDecisionPlayers
  ...
  return input.players.map((originalPlayer) => {
    // ACTIONS.CPP go_team skips !guy_on slots entirely. Keep the stable
    ...
    const expiredOrdinaryIdea = (
      ...
      || originalPlayer.intelligence.move === DRIBBLE_INTELLIGENCE_MOVE
    ...
    if (player.role === "keeper") {
      ...
      const pendingShot = input.players.find((candidate) => (
        ...
        || (
          ...
          && (
            candidate.livePendingShot.kind !== "cross"
            || candidate.livePendingShot.release?.tick < input.nextTick
            || input.visits.findIndex(
              ({ playerId }) => playerId === candidate.id,
            ) < keeperVisitIndex
          )
        )
        || (
          ...
          && candidate.liveFirstTimeIntercept.release !== undefined
          && (
            candidate.liveFirstTimeIntercept.release.tick < input.nextTick
            || input.visits.findIndex(
              ({ playerId }) => playerId === candidate.id,
            ) < keeperVisitIndex
          )
      ...
      if (waitsForShot) {
        ...
        const settled = settlePlayer(current, {
          ballPosition: visit.ballPosition,
          nextTick: input.nextTick,
          possession: visit.possession,
          rngSeed: input.rngSeed,
        });
        const waiting = current.motion.directionMode === 2
          ? {
              ...settled,
              // stand_action's shot_pending guard skips find_zonal_target.
              // A keeper coming out of I_GET_UP therefore reaches process_dir
              // with the retained no-turn mode and old go target intact.
              facing: clone(player.facing),
              target: { ...clone(current.motion.target), z: F32(0) },
              action: createCssoccerActionState({
                tick: input.nextTick,
                playerId: player.id,
              ...
              liveMotion: {
                ...settled.liveMotion,
                target: clone(current.motion.target),
                directionMode: 2,
              },
            }
          : settled;
        const pendingRelease = pendingShot.liveShot
          ?? pendingShot.livePendingShot
          ?? pendingShot.liveFirstTimeIntercept;
        return {
          ...waiting,
          liveMotion: {
            ...waiting.liveMotion,
            ...
            resetAnimationFrame: pendingRelease.release?.tick === input.nextTick
    ...
    const continuingClearedOffsideRunback = player.intelligence.move === 0
      && player.intelligence.count === 0
      && player.action.action.value === CSSOCCER_NATIVE_ACTIONS.RUN
      && player.liveMotion.kind === "offside-runback"
      && player.liveMotion.goCount === 1
      && (
        input.restartResetIdeas
        || (
          visit.possession.owner !== 0
          && (
    ...
    if (continuingClearedOffsideRunback) {
      // computer_play clears tm_off when the opponent owns the ball, while
      // init_match_mode/reset_all_ideas does the same at a restart. Neither
      // clears the already-installed RUN_ACT journey: run_action consumes its
      // final step before find_zonal_target installs and executes a new route.
      const continued = continueSourceSupportJourney(current, {
        ballPosition: visit.ballPosition,
        nextTick: input.nextTick,
        possession: visit.possession,
    ...
    const continuingResetDribble = player.intelligence.move === 0
      ...
      && (
        ...
        || (
          ...
          && typeof player.liveMotion.mustPass === "boolean"
        )
      )
      && (
    ...
    if (continuingResetDribble) {
      ...
      if (continued.liveMotion.goCount <= 0) {
        const planned = planZonalPlayer({
          player: continued,
          motion: continued.liveMotion,
          teamRate: rates.get(player.id),
        ...
        return {
          ...
          liveMotion: {
            ...planned.liveMotion,
            // run_action consumes the last displaced-dribble step before
            // find_zonal_target installs and executes its replacement route.
            goCount: 0,
          },
        };
      }
    ...
    if (continuingSupportRun) {
      ...
      if (continued.liveMotion.goCount <= 0) {
        ...
        return {
          ...
          previousFacing: clone(player.facing),
          velocity: {
            x: F32(stopped.position.x - player.position.x),
            y: F32(stopped.position.y - player.position.y),
            z: F32(stopped.position.z - player.position.z),
          },
          target: { ...clone(continued.liveMotion.target), z: F32(0) },
          intelligence: { special: 0, move: 0, count: 0 },
          liveMotion: {
            ...stopped.liveMotion,
            // run_action consumes the last installed step, then
            // init_stand_act clears I_RUN_ON without changing go_tx/go_ty.
            target: clone(continued.liveMotion.target),
            goStep: continued.liveMotion.goStep,
          },
        };
      }
      return {
    ...
    if (visit.possession.owner === player.nativePlayerNumber) {
      ...
      const continuing = visit.interaction === "hold"
    ...
    return {
      ...
      liveMotion: {
        ...planned.liveMotion,
        // find_zonal_target executes its newly installed go_forward inside
        // this visit, then clears go_cnt before process_dir.
        goCount: 0,
      },
    };
  });
...
export function resolveCssoccerFreePlaySupportIntent(input = {}) {
  ...
  if (!Number.isSafeInteger(input.nextTick) || input.nextTick < 1) {
    throw new TypeError("Free-play support intent nextTick must be a positive integer.");
  }
  ...
  if (!["after-holder", "all", "before-holder"].includes(input.candidateWindow)) {
    throw new TypeError("Free-play support intent candidateWindow is invalid.");
  }
  if (typeof input.holderVisitCompleted !== "boolean") {
    throw new TypeError("Free-play support intent holderVisitCompleted must be boolean.");
  }
  if (typeof input.supportMe !== "boolean") {
    throw new TypeError("Free-play support intent supportMe must be boolean.");
  }
  if (
    !Number.isFinite(input.defensiveLines?.teamA)
    || !Number.isFinite(input.defensiveLines?.teamB)
  ) {
    throw new TypeError("Free-play support intent requires source defensive lines.");
  }
  if (typeof input.justScored !== "boolean") {
    throw new TypeError("Free-play support intent justScored must be boolean.");
  }
  if (typeof input.offsideEnabled !== "boolean") {
    throw new TypeError("Free-play support intent offsideEnabled must be boolean.");
  }
  if (
    !Number.isSafeInteger(input.opponentsNearHolder)
    || input.opponentsNearHolder < 0
    || input.opponentsNearHolder > 11
  ) {
    throw new TypeError("Free-play support intent opponentsNearHolder must be in 0..11.");
  }
  if (
  ...
  // want_pass is one source-global player number. A request from the other
  // team remains visible to pass_decide until process_comments clears it on
  // a later logic tick; it is not scoped to the current holder's team.
  ...
  const expiringRequester = continuing.find(
    sourceSupportRequestClearsDuringVisit,
  );
  const expiringRequesterVisitIndex = expiringRequester === undefined
    ? -1
    : input.visits.findIndex(({ playerId }) => playerId === expiringRequester.id);
  if (expiringRequester !== undefined && expiringRequesterVisitIndex < 0) {
    throw new Error("Free-play support intent lost the expiring requester visit.");
  }
  const run = continuing.length === 0 || expiringRequester !== undefined
      ? selectSourceSupportRun({
        blockedUntilVisitIndex: expiringRequesterVisitIndex,
        candidateWindow: input.candidateWindow,
        controlledPlayerId: input.controlledPlayerId,
        defensiveLines: input.defensiveLines,
        holderVisitCompleted: input.holderVisitCompleted,
        justScored: input.justScored,
        logicCount: input.logicCount,
        nextTick: input.nextTick,
        offsideEnabled: input.offsideEnabled,
        opponentsNearHolder: input.opponentsNearHolder,
        players,
        possession: input.possession,
        rngSeed: input.rngSeed,
        supportMe: input.supportMe,
        takerId: input.takerId,
        visits,
      })
    : null;
  const runRequester = run === null
    ? undefined
    : players.find(({ id }) => id === run.playerId);
  let holderWantPassNativePlayer = 0;
  if (holder !== undefined) {
    const holderIndex = input.visits.findIndex(({ playerId }) => playerId === holder.id);
    const continuingRequester = continuing[0];
    const continuingRequesterIndex = continuingRequester === undefined
      ? -1
      : input.visits.findIndex(({ playerId }) => playerId === continuingRequester.id);
    const runRequesterIndex = runRequester === undefined
      ? -1
      : input.visits.findIndex(({ playerId }) => playerId === runRequester.id);
    if (
      holderIndex < 0
      || (continuingRequester !== undefined && continuingRequesterIndex < 0)
      || (runRequester !== undefined && runRequesterIndex < 0)
    ) {
      throw new Error("Free-play support intent lost source traversal order.");
    }
    const continuingVisibleAtHolder = continuingRequester !== undefined
      && (
        expiringRequester === undefined
        || continuingRequesterIndex >= holderIndex
      );
    const runVisibleAtHolder = runRequester !== undefined
      && runRequesterIndex < holderIndex;
    const requesterAtHolder = continuingVisibleAtHolder
      ? continuingRequester
      : runVisibleAtHolder
        ? runRequester
        : undefined;
    if (requesterAtHolder !== undefined && requesterAtHolder.id !== holder.id) {
      holderWantPassNativePlayer = requesterAtHolder.nativePlayerNumber;
    }
  }
  return deepFreeze({
...
function selectSourceSupportRun({
  ...
  for (const player of ordered) {
    ...
    if (
      visit.index <= blockedUntilVisitIndex
      ||
      (candidateWindow === "before-holder" && visit.index >= holderVisit.index)
      || (candidateWindow === "after-holder" && visit.index <= holderVisit.index)
    ) continue;
    // go_team calls override/offside_rule before intelligence. A player who
    // becomes a potential offside runner takes we_have_ball's run_back branch
    // and cannot install want_pass/I_RUN_ON on this visit.
    if (
      offsideEnabled
      && sourceSupportPlayerIsOffside({
        defensiveLines,
        justScored,
        player,
        visit,
    ...
    const visitOwner = visit.possession.owner;
    if (
      visitOwner === 0
      || visitOwner === player.nativePlayerNumber
      || (visitOwner < 12) !== (player.nativePlayerNumber < 12)
    ) continue;
    const visitHolder = players.find(
      ({ nativePlayerNumber }) => nativePlayerNumber === visitOwner,
    );
    if (visitHolder === undefined) {
      throw new Error("Team journey support run lost the visit-visible holder.");
    }
    if (visit.distance >= CALL_DISTANCE) continue;
    const help = sourceHelpChance({
      ballPosition: visit.ballPosition,
      // FOOTBALL.CPP get_opp_near_ball freezes this global before
      // process_teams. A same-pass collection changes ball_poss, but a later
      // we_have_ball visit still reads the entry holder's pressure count.
      opponentsNearHolder: opponentsNearHolder ?? sourceOpponentCountNearBall({
        ballPosition: visit.ballPosition,
        holder: visitHolder,
        players,
      }),
    ...
    return {
      ...
      target: sourceSupportRunTarget({
        ballPosition: visit.ballPosition,
        holder: visitHolder,
        holderFacing: sourceHolderFacingAtRequesterVisit({
          ballPosition: visit.ballPosition,
          holderAlreadyVisited: holderVisitCompleted
            && candidateWindow === "after-holder"
            && holder.action.tick === nextTick,
          holderIsCentreTaker: holder.id === takerId,
          holder: visitHolder,
          players,
          requester: player,
          rngSeed,
      ...
      wantPassStat: visitHolder.nativePlayerNumber,
    };
  }
  return null;
}

function sourceSupportRequestClearsDuringVisit(player) {
  if (player.intelligence.count <= 1) return true;
  if (player.liveMotion?.kind !== "support-run") return false;
  const offset = player.liveMotion.wantPassOffset;
  if (!Number.isFinite(offset?.x) || !Number.isFinite(offset?.y)) {
    throw new Error(
      `Active support run lost source go_xoff/go_yoff for ${player.id}.`,
    );
  }
  ...
  const facingReset = player.liveMotion.wantPassFaced === true && !requestFacing;
  const routeArrives = player.action.action.value === CSSOCCER_NATIVE_ACTIONS.RUN
    && player.liveMotion.goCount <= 1;
  // intelligence can clear I_RUN_ON before do_action; otherwise run_action's
  // terminal go_forward enters init_stand_act and clears the source-global
  // want_pass during this player's visit. Later teammates may request it.
  return facingReset || routeArrives;
}

function sourceSupportPlayerIsOffside({
  defensiveLines,
  justScored,
  player,
  visit,
}) {
  if (justScored || visit.canBeOffside !== 1) return false;
  const margin = CSSOCCER_NATIVE_GAMEPLAY_PROFILE.constants.prat.value;
  if (player.nativeTeamSlot === "A") {
    return visit.possession.owner < 12
      && player.position.x > 640
      && player.position.x > F32(defensiveLines.teamB + margin);
  }
  return (visit.possession.owner > 11 || visit.possession.owner === 0)
    && player.position.x < 640
    && player.position.x < F32(defensiveLines.teamA - margin);
}
...
function sourceHelpChance({
  ...
  // INTELL.CPP help_chance replaces the ordinary pressure calculation while
  // await_set_kick's user_taker path has raised the source-global support_me.
  if (supportMe) return Math.trunc(512 - (range * range));
...
function sourceHolderFacingAtRequesterVisit({
  ballPosition,
  holderAlreadyVisited,
  holderIsCentreTaker,
  holder,
  players,
  requester,
  ...
  // The open-play after-holder projection already contains the completed
  // holder visit, including run_action and process_dir. Replaying that route
  // here would turn the holder twice before the later teammate's
  // kicking_dist/help request observes its facing.
  if (holderAlreadyVisited) return holder.facing;
  const holderRunsBeforeRequester = holder.nativePlayerNumber < requester.nativePlayerNumber;
  if (!holderRunsBeforeRequester) return holder.facing;

  // On the opening centre pass, the taker's earlier source visit has already
  // installed KICK_ACT and dir_mode=2. kicking_dist therefore reads the
  // retained centre facing; it must not replay ordinary standing dir_ball.
  if (holderIsCentreTaker) return holder.facing;

  const continuesHeldBallRun = holder.action.action.value === CSSOCCER_NATIVE_ACTIONS.RUN
    && holder.intelligence.move === DRIBBLE_INTELLIGENCE_MOVE
    && holder.intelligence.count > 0
    && holder.liveMotion?.kind === "run-with-ball";
  if (continuesHeldBallRun) {
    const holderVisit = visits.get(holder.id);
    if (holderVisit === undefined) {
      throw new Error("Source support facing lost the holder visit.");
    }
    const continuing = holder.intelligence.count > 1;
    const dribble = continuing
      ? { target: clone(holder.liveMotion.target) }
      : selectCssoccerDribbleRun({
          ball: {
            x: holderVisit.ballPosition.x,
            y: holderVisit.ballPosition.y,
          },
          pitch: {
            length: PITCH_LENGTH,
            ratio: CSSOCCER_NATIVE_GAMEPLAY_PROFILE.constants.prat.value,
          ...
          player: {
            ...
            distance: holderVisit.distance,
          },
          players: players.filter(({ active }) => active).map((player) => ({
            nativePlayer: player.nativePlayerNumber,
            action: player.action.action.value,
            distance: visits.get(player.id).distance,
            on: player.active,
  ...
  // A standing holder with dir_mode 1 therefore turns toward the same-visit
  // ball before a later we_have_ball requester calls kicking_dist.
  if (holder.liveMotion?.directionMode === 1) {
    return turnSourceFacing({
      facing: holder.facing,
      target: {
        x: F32(ballPosition.x - holder.position.x),
        y: F32(ballPosition.y - holder.position.y),
      },
      maxTurnRadians: projectCssoccerMotionSourceProfile(
        CSSOCCER_NATIVE_GAMEPLAY_PROFILE,
        { teamRate: holder.liveMotion.teamRate },
...
function currentVisitMap(visits, players) {
  ...
  if (!Array.isArray(visits) || visits.length !== activePlayers.length) {
    throw new Error("Team journey continuation requires one source-order visit per active player.");
  }
  const byId = new Map();
  for (const [index, visit] of visits.entries()) {
    ...
    if (visit.canBeOffside !== 0 && visit.canBeOffside !== 1) {
      throw new TypeError(`Team journey visit ${visit.playerId} offside eligibility is invalid.`);
    }
...
function planZonalPlayer(
  ...
  return updatePlayer(player, {
    ...
    liveMotion: {
      ...
      // get_there_time can retain RUN_ACT while stop_and_face turns in place.
      // go_forward clears this only once the route is aligned enough to move.
      goStop,
      goCount,
      goDisplacement,
