...
- Start every exact-parity iteration with `node tools/run-differential-frontier.mjs --continue`. Treat its compact packet as the control loop: work only on its checked field and named browser producer, do not rerun an unchanged runtime snapshot, and use the public capture/publisher only after one generic candidate is ready. Do not replace this action with ignored compare/raw/source-search helpers.
- When the frontier packet's named producer still leaves a Watcom function, float-store, or runtime-global question, use `node tools/run-compiled-path-check.mjs --function <name> --object <module> --symbol <name[:type]>`. This nested action owns the current Exact tick, bindings, static inspection, and short read-only probe. Do not hand-edit oracle capture ranges or hard-code a probe tick/address.
